...
function CommonUINotice:addNotice(string, txtColor, font)
    ...
    self.noticeText = CommonHelper:newBMFontLabel(string,nil,nil,display.width-40)
...
function CommonUINotice:addNoticeWithItem(itemId, itemNum)
    ...
    self.noticeText = CommonHelper:newBMFontLabel(CommonHelper:convertToCurrentLanguage(32010), cc.TEXT_ALIGNMENT_LEFT)
    self.noticeText._saveScale = self.noticeText:getScaleX()
    self.noticeText:setAnchorPoint(cc.p(1, 0.5))
    self.noticeText2 = CommonHelper:newBMFontLabel("x "..CommonHelper:formatNumber(itemNum, globalConfig.NumberFormat.FORMAT_NUMBER_FOUR))
